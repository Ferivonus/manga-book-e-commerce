...
// --- Tip Güvenliği İçin Interface'ler ---
...
interface MangaWithVolumes extends Manga {
...
export default function VolumeDetailPage({ 
  ...
  const [manga, setManga] = useState<MangaWithVolumes | null>(null);
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      setIsLoading(true);
      try {
        // Backend'den seriyi ve içindeki ciltleri çekiyoruz
        ...
        const mangaData = data as MangaWithVolumes;
        setManga(mangaData);

        // URL'deki volumeSlug'a göre doğru cildi eşleştiriyoruz
  ...
  const discountPercentage = currentVolume.originalPrice 
    ? Math.round(((currentVolume.originalPrice - currentVolume.price) / currentVolume.originalPrice) * 100) 
    : 0;
    ...
    <div className="bg-background min-h-screen pb-32 transition-colors duration-500">
      ...
      <nav className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        <ol className="flex items-center space-x-3 text-[10px] font-black uppercase tracking-[0.2em] text-foreground/30">
          ...
          {/* ROTA DÜZELTİLDİ: Artık slug kullanıyor */}
          <li><Link href={`/manga/${manga.slug}`} className="hover:text-primary transition-colors italic">{manga.title}</Link></li>
          <li className="opacity-40"><ChevronRightIcon className="h-3 w-3" /></li>
          <li className="text-primary truncate">Cilt {currentVolume.volumeNumber}</li>
        </ol>
      </nav>
          ...
          <div className="lg:col-span-5 relative group">
            <div className="relative aspect-[2/3] w-full overflow-hidden rounded-[4rem] bg-foreground/5 shadow-[0_50px_100px_rgba(0,0,0,0.12)] ring-1 ring-foreground/10 group-hover:ring-primary/40 transition-all duration-700">
              ...
              <div className="absolute top-10 left-10 flex flex-col gap-4 z-10">
                <span className="bg-primary text-white px-5 py-2 rounded-2xl text-[10px] font-black shadow-2xl uppercase tracking-widest border border-white/20">
                  CİLT {currentVolume.volumeNumber}
                </span>
          ...
          <div className="mt-16 lg:mt-0 lg:col-span-7">
            <div className="border-b border-foreground/5 pb-12">
              ...
              <div className="flex flex-wrap items-center gap-6">
                <p className="text-2xl text-foreground/30 font-bold italic tracking-tight">
            ...
            {/* Fiyat ve Sepet Aksiyonu */}
            <div className="py-12">
              <div className="flex items-end gap-8 mb-12">
                <div className="flex flex-col">
                  {currentVolume.originalPrice && (
                    <span className="text-2xl text-foreground/20 line-through font-bold mb-[-5px] italic">₺{currentVolume.originalPrice.toFixed(2)}</span>
                  )}
                  <span className="text-7xl font-black text-accent tracking-tighter italic leading-none">₺{currentVolume.price.toFixed(2)}</span>
                </div>
                <div className="mb-2">
                  <div className="flex items-center gap-2 bg-secondary/5 px-4 py-2 rounded-2xl border border-secondary/10 text-secondary">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest">Stokta Mevcut</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="flex items-center justify-between bg-foreground/[0.03] rounded-[2rem] p-2 sm:w-48 border border-foreground/5 shadow-inner">
                  ...
                  <button onClick={() => setQuantity(q => Math.min(currentVolume.stock, q + 1))} className="h-14 w-14 flex items-center justify-center text-foreground/20 hover:text-primary transition-all text-2xl font-black">+</button>
                </div>
            ...
            <div className="bg-foreground/[0.02] rounded-[3.5rem] p-10 sm:p-12 border border-foreground/5 relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-y-10 gap-x-12">
                <InfoItem label="YAYINEVİ" value={currentVolume.publisher} />
            ...
            <div className="mt-16 flex flex-col sm:flex-row sm:items-center justify-between px-6 gap-4">
               ...
               {/* LİNK DÜZELTİLDİ: Artık slug kullanıyor */}
              ...
              {manga.volumes.filter(v => v.slug !== currentVolume.slug).map(vol => (
                <Link key={vol.id} href={`/manga/${manga.slug}/${vol.slug}`} className="flex-shrink-0 group flex flex-col items-center">
                  <div className="relative h-44 w-32 rounded-3xl overflow-hidden ring-1 ring-foreground/10 group-hover:ring-primary/50 group-hover:-translate-y-2 transition-all duration-500 shadow-lg">
                    <Image src={vol.imageUrl} alt={vol.title} fill className="object-cover transition-transform duration-700 group-hover:scale-110" />
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <span className="text-white text-[9px] font-black tracking-widest">OKU</span>
                    </div>
                  </div>
                  <p className="text-center text-[9px] font-black mt-4 text-foreground/30 uppercase tracking-tighter">CİLT {vol.volumeNumber}</p>
                </Link>
              ))}
            </div>
          </div>
