...
export default function Home() {
  const [mangas, setMangas] = useState<HomeManga[]>([]);
  const [bestSellers, setBestSellers] = useState<HomeManga[]>([]);
  ...
  useEffect(() => {
    const fetchHomeData = async () => {
      ...
      try {
        ...
        setMangas(allSeries as unknown as HomeManga[]);

        const { bestSellers: featuredData } = await getFeaturedMangas();
        setBestSellers(featuredData as unknown as HomeManga[]);
      } catch (error) {
    ...
    <div className="relative bg-background transition-colors duration-500 overflow-hidden">
      ...
      <section className="relative w-full h-[95vh] min-h-[800px] overflow-hidden bg-foreground/5 -mt-24">
        {heroMangas.map((manga, idx) => {
          const firstVol = manga.volumes?.[0];
          return (
            <div 
              key={manga.id} 
            ...
            >
              ...
              <div className="absolute inset-0 z-0">
                <Image
                  src={firstVol?.imageUrl || '/placeholder.png'}
                  alt={manga.title}
                  fill
                  className="object-cover object-[center_20%] transition-transform duration-[20s] scale-110 group-hover:scale-100"
                  priority={idx === 0}
                />
                  ...
                  <div className="max-w-4xl">
                    ...
                    <div className="flex flex-wrap items-center gap-12">
                      <div className="flex flex-col">
                        <span className="text-6xl font-black text-accent tracking-tighter italic">₺{Number(firstVol?.price || 0).toFixed(2)}</span>
        ...
        <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex items-center gap-8 z-20">
          <button onClick={() => setCurrentSlide(prev => prev === 0 ? heroMangas.length - 1 : prev - 1)} className="p-4 rounded-full bg-foreground/5 backdrop-blur-md border border-white/10 text-foreground/40 hover:text-primary hover:scale-110 transition-all">
            ...
            {heroMangas.map((_, idx) => (
              <button
                ...
                className={`transition-all duration-700 rounded-full h-1.5 ${
                  currentSlide === idx ? 'bg-primary w-16 shadow-[0_0_20px_rgba(var(--primary),0.6)]' : 'bg-foreground/10 w-2 hover:bg-foreground/30'
                }`}
              />
            ))}
          </div>
          <button onClick={() => setCurrentSlide(prev => prev === heroMangas.length - 1 ? 0 : prev + 1)} className="p-4 rounded-full bg-foreground/5 backdrop-blur-md border border-white/10 text-foreground/40 hover:text-primary hover:scale-110 transition-all">
      ...
      <section className="container mx-auto px-6 sm:px-12 lg:px-20 py-40 relative z-20">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 border-b border-foreground/5 pb-14 mb-24">
          <div>
            ...
            <p className="mt-6 text-xl text-foreground/40 font-medium italic">Editörlerin seçtiği, kütüphanenizin baş köşesini hak eden başyapıtlar.</p>
          </div>
          <Link href="/populer" className="group text-[12px] font-black text-primary uppercase tracking-[0.3em] flex items-center gap-4 hover:underline underline-offset-[12px] decoration-2 transition-all">
              ...
              <div key={manga.id} className="group flex flex-col">
                ...
                <div className="relative aspect-[2/3] rounded-[4rem] overflow-hidden bg-foreground/5 shadow-2xl transition-all duration-700 ring-1 ring-foreground/5 group-hover:ring-primary/40 group-hover:-translate-y-6 group-hover:shadow-[0_60px_100px_rgba(var(--primary),0.15)]">
                  ...
                  <div className="absolute top-10 left-10 z-10 flex flex-col gap-3">
                    ...
                    <span className="bg-background/80 backdrop-blur-md text-foreground px-4 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest shadow-lg border border-white/10">
                ...
                <div className="mt-12 px-6 text-center lg:text-left">
                  <div className="flex items-center justify-center lg:justify-between mb-4">
                    <span className="text-[10px] font-black text-secondary uppercase tracking-[0.3em]">{manga.category?.name}</span>
                  ...
                  <div className="pt-8 border-t border-foreground/5 flex items-center justify-between">
                    <p className="text-3xl font-black text-accent tracking-tighter italic leading-none">₺{Number(firstVol?.price || 0).toFixed(2)}</p>
